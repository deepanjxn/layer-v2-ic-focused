...
const BentoGrid = ({ children, className }: BentoGridProps) => {
  return (
    <div className={cn("grid grid-cols-1 md:grid-cols-6 lg:grid-cols-6 gap-0 rounded-3xl border", className)}>
      {children}
    </div>
  )
}
...
const BentoCard = ({ id, children, className }: BentoCardProps) => {
  return (
    <div id={id} className={cn("relative overflow-hidden p-4 sm:p-8", className)}>
      {children}
    </div>
  )
}
...
const BentoTitle = ({ children, className }: BentoTitleProps) => {
  ...
  return (
    <h3 className={cn("text-left text-xl tracking-tight text-foreground md:text-2xl md:leading-snug", className)}>
      {children}
    </h3>
  )
}
...
const BentoDescription = ({ children, className }: BentoDescriptionProps) => {
  if (!children) return null
  return (
    <p className={cn("text-left text-sm md:text-sm font-normal text-text-secondary mx-0 my-2 max-w-sm", className)}>
      {children}
    </p>
  )
}
